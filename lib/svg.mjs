...
/**
 * Compose an inline-ready SVG icon from a SYMBOL node and its children.
 *
 * Takes the parent SYMBOL's dimensions as the viewBox, positions each child
 * via <g transform="translate(x, y)">, extracts <path> elements from
 * pre-extracted SVG files, and replaces #ffffff with currentColor.
 *
 * @param {object} symbolNode - The parent SYMBOL nodeChange
 * @param {object[]} childNodes - Array of { node, transform: {x, y} } for each child
 ...
 */
export function composeSvgIcon(symbolNode, childNodes, getSvgFile) {
  const size = symbolNode.size || {};
  const w = size.x || 0;
  const h = size.y || 0;
  ...
  for (const { node, transform } of childNodes) {
    ...
    // Extract <path> elements from the SVG file
    const paths = [];
    const pathRe = /<path\b[^>]*\/>/g;
    let m;
    while ((m = pathRe.exec(svgContent)) !== null) {
      // Replace #ffffff / #fff with currentColor
      let path = m[0]
    ...
    if (x === 0 && y === 0 && !opAttr) {
      ...
      groups.push(`<g transform="translate(${x}, ${y})"${opAttr}>${paths.join('')}</g>`);
    }
  }

  if (groups.length === 0) return null;

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" fill="currentColor">\n  ${groups.join('\n  ')}\n</svg>`;
}
